    ...
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-4 py-16 relative overflow-hidden">
        ...
        <div className="mb-8 text-center">
          ...
          <h1 className="text-4xl sm:text-5xl font-extrabold tracking-tight text-foreground mb-3">How can I help you today?</h1>
          <p className="text-muted-foreground text-lg max-w-md mx-auto">Ask me anything about your inbox, schedule, repos, or messages.</p>
        </div>
        {/* Composer card */}
        <div className="w-full max-w-2xl">
          <div className="relative rounded-2xl border border-border/60 bg-white dark:bg-card shadow-lg dark:shadow-xl dark:shadow-black/20 ring-1 ring-border/20 transition-all focus-within:ring-2 focus-within:ring-amber-400/40 dark:focus-within:ring-primary/30 focus-within:border-amber-400/60 dark:focus-within:border-primary/40 focus-within:shadow-xl focus-within:shadow-amber-200/20 dark:focus-within:shadow-primary/5">
            ...
            <div className="absolute bottom-0 left-0 right-0 flex items-center justify-between px-4 py-3 border-t border-border/30">
              ...
              <div className="flex items-center gap-2">
                {integrations.map((item, i) => {
                  ...
                  return (
                    <div key={i} title={item.label} className="flex items-center gap-1.5 px-2 py-1 rounded-lg bg-muted/40 hover:bg-muted/70 border border-transparent hover:border-border/40 transition-all cursor-default">
              ...
              <div className="flex items-center gap-2">
                ...
                <Button size="icon-sm" disabled={!value.trim() || loading} onClick={handleSubmit} className="h-8 w-8 rounded-xl shadow-md shadow-primary/20 disabled:opacity-30 disabled:shadow-none transition-all hover:scale-105">
            ...
            {/* Character hint */}
            {value.length > 0 && (
              <p className="text-[10px] text-muted-foreground/50 text-right mt-1.5 pr-1">{value.length} chars</p>
            )}
          </div>
        </div>
        {/* Response block */}
              ...
              {response && (
                <>
                  ...
                  <div className="flex items-center justify-end gap-2 mt-4 pt-3 border-t border-border/20">
                    <Button variant="ghost" size="sm" onClick={() => { navigator.clipboard.writeText(response); toast.success("Copied to clipboard"); }} className="text-[10px] font-semibold h-7 rounded-lg text-muted-foreground hover:text-foreground hover:bg-accent/40 cursor-pointer animate-in fade-in">
                      Copy Response
                    </Button>
                    <Button variant="ghost" size="sm" onClick={() => { setResponse(null); setValue(""); }} className="text-[10px] font-semibold h-7 rounded-lg text-muted-foreground hover:text-destructive hover:bg-destructive/10 cursor-pointer animate-in fade-in">
                      Clear
                    </Button>
                  </div>
                </>
        ...
        <div className="mt-8 w-full max-w-2xl">
          <p className="text-xs font-semibold text-muted-foreground/60 uppercase tracking-wider mb-3 text-center">Try asking</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
            {suggestions.map((s, i) => {
              const Icon = s.icon;
              return (
                <button key={i} onClick={() => handleSuggestion(s.label)} className={`flex items-center gap-3 px-4 py-3 rounded-xl border bg-white dark:bg-card text-left text-sm font-medium text-foreground/80 transition-all duration-150 hover:text-foreground hover:-translate-y-0.5 hover:shadow-md dark:hover:shadow-black/20 shadow-sm ${s.bg}` }>
